                    ...
                    double *gyro_x, double *gyro_y, double *gyro_z)//контроль и генерация значений для полета
{
    ...
    {
        *gyro_x =0.0;
        *gyro_y = 0.0;//для гироскопа
        *gyro_z = 0.0;
        *az = 3.0;
    } else if (i < total_time - 10)
    {
        *gyro_x = 0.0;
        *gyro_y = -3;//для гироскопа
        *gyro_z = 0.0;
        *az = 0.0;
        *ay = 1.0;
...
void compute_magnetometer(double pitch, double B0, double *Bx, double *By, double *Bz) {
    double pitch_rad = pitch * M_PI / 180.0;  // Перевод тангажа в радианы

    // Рассчёт компонентов магнитного поля
    *Bx = B0 * cos(pitch_rad);
    *By = 0;  // Нет бокового наклона
    *Bz = B0 * sin(pitch_rad);

    // Перевод в милигауссы
    *Bx *= UT_TO_MG;
    *By *= UT_TO_MG;
    *Bz *= UT_TO_MG;
}
...
int flight()
{
    ...
    for (int i = 0; i <= total_time; i++)//работа со значениями и запись в БД
    {
        ...
        char sql[256];
        snprintf(sql, sizeof(sql), "INSERT INTO model_flight VALUES (%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f)",i, lon,lat,roll, pitch, yaw,vx,vy,vz,mx,my,mz,z);
